...
module.exports.run = function (f, pg, mo, redis, pg2) {
    ...
    let sql = `INSERT INTO im_系统消息内容表(消息id, 会话类型, 消息类型, 消息内容, 发送时间, 发送群体, 状态, 录入人, 录入时间, 备注)
    ...
    if (result.影响行数 != 1) {
        ...
        if (sysFlag == '系统单发') {
            ...
            //系统消息接收表入库
            let sql = `INSERT INTO im_系统消息接收表 (接收者账号, 会话类型, 消息类型, 消息id, 状态, 接收时间,录入人,录入时间)
        ...
        if (sysFlag == '系统群发') {
            ...
            let sql = `INSERT INTO im_系统消息接收表 (接收者账号, 会话类型, 消息类型, 消息id, 状态, 接收时间,录入人,录入时间) VALUES`;
            let sql2 = ``;
            ...
            for (let i = 0; i < redisKeys.length; i++) {
                ...
                console.log(user[1]);
                if (f.group != '所有') {
                    if (user[1] != '用户' && user[1] != '商家') continue;
                } else {
                    //去除客服
