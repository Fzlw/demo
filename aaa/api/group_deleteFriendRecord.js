...
module.exports.run = function(body, pg, mo, redis, pg2) {
	...
	sql = `insert into 日志_非钱表(账号,时间,ip,类别,状态,录入人,录入时间,备注)values('${f.账号}','${body.date}','${body.ip}','删除好友申请记录','数据提交','系统','${body.date}','入参：${JSON.stringify(body)} 返回参数：${JSON.stringify(data)}')`;
...
function middle(pg,f,data,body,redis){
    ...
    sql = "update 群_好友群记录操作表  set 状态 = '已删除' where id = $1 and 操作账号 = $2 and 类别 = '好友'";
    result = pgdb.query(pg, sql,[f.记录id,f.账号]);
	if (result.状态 != '成功') {
		data.状态 = "网络异常，请稍后再试";
